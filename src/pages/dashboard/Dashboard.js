...
const Dashboard = () => {
    ...
    const pendingRequests = reservations.filter((r) => r.status === 'Pending' || r.status === 'Request Approval' || r.status === 'To Pay').length;
    ...
    return (
        <div className="dashboard-page">
            ...
            <motion.div className="quick-actions-bar card flex-between align-center px-6 py-4" initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ borderRadius: '16px', background: 'rgba(255,255,255,0.7)', backdropFilter: 'blur(10px)', border: '1px solid rgba(139, 111, 92, 0.1)' }}>
            ...
            <div className="charts-grid" style={{ gridTemplateColumns: (!widgetPrefs.chartReservationTrends || !widgetPrefs.chartPopularOutfits) ? '1fr' : '2fr 1fr' }}>
                {widgetPrefs.chartReservationTrends && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.3 }} className="chart-card card">
                ...
                {widgetPrefs.chartPopularOutfits && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.4 }} className="chart-card card">
                        <div className="card-header">
            ...
            <div className="widgets-grid" style={{ gridTemplateColumns: (!widgetPrefs.widgetLowStock || !widgetPrefs.widgetRecentCustomers) ? '1fr' : 'repeat(auto-fit, minmax(300px, 1fr))' }}>
                {widgetPrefs.widgetLowStock && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="widget card">
                        <div className="card-header">
                        ...
                        <div className="widget-list">
                            ...
                            {lowStockItems.map((item) => (
                                <div className="widget-item alert-item" key={item.id}>
                                    <div className="item-icon-bg alert">
                                        <AlertTriangle size={18} className="text-danger" />
                                    </div>
                                    <div className="item-details">
                                        <h4>{item.item}</h4>
                                        <p>Size {item.size} &middot; Only {item.available} left</p>
                                    </div>
                ...
                {widgetPrefs.widgetRecentCustomers && (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="widget card">
                        <div className="card-header">
                        ...
                        <div className="widget-list">
                            ...
                            {recentCustomers.map((c, i) => (
                                <div className="widget-item" key={c.id}>
                                    <div className="avatar-small" style={{ backgroundColor: `hsl(${200 + i * 40}, 50%, 50%)` }}>{(c.name || 'U')[0]}</div>
                                    <div className="item-details">
                                        <h4>{c.name}</h4>
                                        <p>{c.lastActive}</p>
                                    </div>
                ...
                {widgetPrefs.widgetWeather && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.7 }} className="weather-insight-card widget card">
                        <div className="card-header">
            ...
            <div className="command-center-grid mt-6" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '1.5rem' }}>
                {widgetPrefs.widgetLogistics && (
                    <motion.div className="card logistics-widget" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.8 }}>
                        <div className="card-header">
                        ...
                        <div className="logistics-timeline mt-4">
                            {todayLogistics.length === 0 ? (
                                <div className="p-8 text-center text-secondary bg-cream rounded-xl">No pickups or returns scheduled for today.</div>
                            ) : (
                                todayLogistics.map((item, idx) => (
                                    <div className={`logistics-row ${item.actionType.toLowerCase()}`} key={idx}>
                                        ...
                                        <div className="logistics-info">
                                            ...
                                            <p>{item.productName || item.outfit} &middot; {item.size}</p>
                                        </div>
                                    </div>
                                ))
                            )}
                ...
                {widgetPrefs.widgetActivityFeed && (
                    <motion.div className="card activity-feed-widget" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.9 }}>
                        <div className="card-header">
                            <div className="flex align-center gap-2">
                        ...
                        <div className="activity-pulse-list mt-4">
                            {activityFeed.map((item, idx) => (
                                <div className="activity-pulse-item" key={idx}>
