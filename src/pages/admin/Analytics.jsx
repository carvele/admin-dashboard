...
const StatCard = ({ title, value, change, icon: Icon, trend, tooltip }) => (
  <div className="card stat-card" title={tooltip || ''}>
    <div className="stat-header">
      ...
      <div className={`stat-icon ${trend === 'up' ? 'text-success' : trend === 'down' ? 'text-danger' : ''}`}>
        <Icon size={20} />
      </div>
    </div>
    <div className="stat-value">{value}</div>
    <div className={`stat-change ${trend === 'up' ? 'text-success' : trend === 'down' ? 'text-danger' : 'text-secondary'}`}>
...
const Analytics = () => {
  ...
  useEffect(() => {
    ...
    return () => { unsubR(); unsubC(); unsubCat(); unsubConv(); };
  }, []);

  // Compute total revenue purely from reservation data (Completed OR Confirmed)
  let totalRev = 0;
  const completedOrConfirmed = reservations.filter(r => r.status === 'Completed' || r.status === 'Confirmed' || r.status === 'Approved');
  ...
  const getMonthDelta = (list, dateField) => {
    ...
    const thisMonth = list.filter(item => {
      const d = new Date(item[dateField] || (item.createdAt?.seconds ? item.createdAt.seconds * 1000 : 0));
  ...
  const popularOutfits = Object.entries(outfitCounts).sort((a,b) => b[1] - a[1]).slice(0, 4).map(([name, count]) => {
     const item = catalog.find(c => c.name === name);
     return { name, value: count, revenue: item ? item.price * count : 0 };
  });
  const actPopular = popularOutfits.length > 0 ? popularOutfits : [{name:'No data', value:0, revenue:0}];
    ...
    if (dateRange === '30d') {
      ...
      reservations.forEach(r => {
        if (r.date || r.createdAt) {
          const d = new Date(r.date || (r.createdAt?.seconds ? r.createdAt.seconds * 1000 : r.createdAt));
  ...
  return (
    <div className="page-container">
      ...
      <div className="analytics-grid-4">
        <StatCard title="Total Revenue" value={`₱${totalRev.toLocaleString()}`} change={revDelta.text} trend={revDelta.trend} icon={TrendingUp} tooltip={`Derived from ${completedOrConfirmed.length} completed/confirmed reservations`} />
        <StatCard title="New Customers" value={customers.length || 0} change={custDelta.text} trend={custDelta.trend} icon={Users} tooltip={`${customers.length} registered customers`} />
        <StatCard title="Reservations" value={reservations.length || 0} change={resDelta.text} trend={resDelta.trend} icon={ShoppingBag} tooltip={`${reservations.length} total reservations`} />
        <StatCard title="AR Try-Ons" value={catalog.filter(p => p.tags && p.tags.includes('AR Try-On')).length} change={`${catalog.filter(p => p.tags && p.tags.includes('AR Try-On')).length} products`} trend="up" icon={Eye} />
      </div>
          ...
          <div className="chart-container" style={{height: 300}}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={reservationTrends} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                ...
                />
                <Area type="monotone" dataKey="reservations" stroke="var(--accent)" strokeWidth={3} fillOpacity={1} fill="url(#colorRes)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
          ...
          <div className="chart-container" style={{height: 300}}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={sortedConvRates} margin={{ top: 20, right: 30, left: 0, bottom: 0 }}>
                ...
                <Bar dataKey="reserved" name="Reservations" fill="var(--beige)" radius={[4, 4, 0, 0]} maxBarSize={40} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
              ...
              <tbody>
                {actPopular.map((item, idx) => (
                  <tr key={idx}>
                    ...
                    <td className="text-right font-medium">₱{(item.revenue || 0).toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-4 pt-0">
             <div className="demo-bar-group">
                <div className="flex-between text-sm mb-1">
                  ...
                  <span className="font-medium">
                    {customers.length > 0 ? Math.round(((customers.length - Object.keys(outfitCounts).length) / customers.length) * 100) || 50 : 0}% /
                    {customers.length > 0 ? Math.round((Object.keys(outfitCounts).length / customers.length) * 100) || 50 : 0}%
                  </span>
                </div>
                <div className="demo-bar">
                  <div className="demo-fill" style={{
             ...
             <div className="demo-bar-group mt-4">
                <div className="flex-between text-sm mb-1">
                  ...
                  <span className="font-medium">
                    {reservations.length > 0 ? Math.round((reservations.filter(r => r.status === 'Completed').length / reservations.length) * 100) : 0}%
                  </span>
                </div>
                <div className="demo-bar">
                  <div className="demo-fill" style={{
