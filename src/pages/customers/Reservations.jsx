...
const Reservations = () => {
  ...
  const handleMarkRefundDisbursed = async (res, method, referenceNumber, notes = null) => {
    const ref = (referenceNumber || refundReferenceNumber || '').trim();
    const m = method || refundDisbursementMethod;
    const n = (notes != null ? notes : refundNotes || '').trim();
    if (!ref) {
    ...
    try {
      await markRefundDisbursed(res.docId || res.id, m, ref, n || null);
      // Optimistic UI update.
      setViewModal((prev) => prev ? { ...prev, paymentStatus: 'Refunded' } : prev);
      toast.success(`Refund disbursement recorded for ${res.displayId || res.id}`);
