...
-- POS v1 writer-audit gate (implementation_plan.md, "Writer audit" step): every
-- current writer of public.reservations must explicitly set purchase_mode and
-- sales_channel before Migration-2 can safely flip those columns to NOT NULL
-- with defaults. Confirmed live (pg_proc scan across all functions, RLS INSERT
-- policy check, grep of both repos' client code and Edge Functions): the only
-- writers are five RPCs. record_boutique_sale's 5-arg overload already sets
-- both (PR #117); the 3-arg legacy overload is frozen by design and is
-- handled separately (it must be dropped before Migration-2 -- see the
-- updated implementation_plan.md, not this migration). The remaining three
-- writers are fixed here.
--
-- create_admin_reservation is staff booking a reservation on a customer's
-- behalf via the admin dashboard -- distinct from both self-service mobile
-- checkout and an in-person walk-in sale, so sales_channel needs a third
-- value rather than overloading 'mobile' or 'boutique_pos' with a meaning
-- neither actually has.
ALTER TABLE public.reservations DROP CONSTRAINT chk_sales_channel;
ALTER TABLE public.reservations
  ADD CONSTRAINT chk_sales_channel CHECK (
    sales_channel IS NULL OR sales_channel IN ('mobile','boutique_pos','admin_assisted')
  );

-- ---------- create_reservation: mobile app, single-item checkout ----------
...
-- ---------- create_reservation_multi: mobile app, cart checkout ----------
...
-- ---------- create_admin_reservation: admin dashboard, staff-assisted booking ----------
